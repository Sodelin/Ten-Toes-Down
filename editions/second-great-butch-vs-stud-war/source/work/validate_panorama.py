...
from build_panorama import load_data,plain,CHORUS,CHORUS_LABEL,load_history,HISTORY_INTRO
...
def strip_furniture(s):
    return '\n'.join(line for line in s.splitlines()if line.strip()not in {CHORUS_LABEL,*[p for row in CHORUS for p in row]} and not re.match(r'^(TEN TOES DOWN  /  PANORAMA OF GAMES|VOLUME \d$|THE GAMES ARE REAL\. THE SUMMIT IS FICTION\.|\d / \d+$)',line.strip()))
...
alltexts=[];allparas=[];chorus_counts={}
...
for v in volumes:
    ...
    chorus_counts[path.name]=[len(re.findall(r'\bniggas\b',t,re.I))for t in texts]
    assert min(chorus_counts[path.name])>=15,(path.name,'Chorus quota not met')
    ...
    if v['number']==1:
        hist_index={h['id']:h for h in json.loads((OUT/'history-index.json').read_text(encoding='utf8'))}
        for h in load_history():
            assert h['id']in texts[hist_index[h['id']]['page']-1],(h['id'],'Wrong history page')
            for s in h['paragraphs']+h['fiction']+[h['question'],h['caveat']]:
                assert norm(s)in joined,(h['id'],'Missing history text',s[:80]);report['paragraphs_checked']+=1
        for s in HISTORY_INTRO:assert norm(s)in joined,('History introduction missing',s[:80])
        report['historical_dossiers_checked']=len(hist_index)
    ...
    for name in z.namelist():
        if name.endswith('.xhtml'):
            if not name.endswith('/nav.xhtml'):
                assert len(re.findall(r'\bniggas\b',z.read(name).decode('utf8'),re.I))>=15,(name,'Chapter chorus missing')
...
report['page_chorus_quota']={name:{'pages':len(counts),'minimum_exact_plural_occurrences':min(counts),'maximum_exact_plural_occurrences':max(counts),'every_page_at_least_15':all(c>=15 for c in counts)}for name,counts in chorus_counts.items()}
report['omnibus_every_page_at_least_15']=all(len(re.findall(r'\bniggas\b',p.extract_text()or'',re.I))>=15 for p in omni.pages)
