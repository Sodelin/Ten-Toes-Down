...
TITLE='The Ten Toes Down Panorama of Games for Lillian - The Tellings Cut'
DATE='2026-09-20'
...
class BookDoc(SimpleDocTemplate):
    def __init__(self,*a,volume=1,**kw):
        super().__init__(*a,**kw);self.volume=volume;self.locations={};self.case_ends={}
...
def make_story(v,front,known_pages,split_games=()):
    n=v['number'];st=[]
    st.extend([Spacer(1,25),para('TEN TOES DOWN PRESENTS / THE TELLINGS CUT','kicker'),para('The Panorama of Games for Lillian','cover'),para(f'VOLUME {n} OF SIX','kicker'),para(v['title'],'h1'),para(v['subtitle'],'sub'),Spacer(1,14),para('Forty real games. Aiden has made it everybody’s problem.','body'),para('Adult comic edition: reclaimed in-group language, profanity, fictional cocaine references and sexual innuendo.','small'),para('New ChatGPT prose under Nolan’s creative direction.','small'),para('Tellings Cut 2.0 - 20 September 2026','small'),PageBreak()])
    ...
    for i,g in enumerate(v['games']):
        ...
        st.append(Spacer(1,6));st.append(para('AIDEN AFTER HOURS / '+g['case_title'],'kicker'))
...
def write_book(v,front):
    path=OUT/f'Panorama-Volume-{v["number"]:02d}.pdf';loc={};split_games=set()
    for attempt in range(3):
        ...
        doc.build(make_story(v,front,loc,split_games),onFirstPage=furniture,onLaterPages=furniture)
        if attempt==0:split_games={gid for gid,start in doc.locations.items()if doc.case_ends[gid]>start}
        if attempt==2 and loc!=doc.locations:raise AssertionError('Index pagination did not converge')
        loc=doc.locations
...
def all_sections(volumes,front):
    ...
    for v in volumes:
        ...
        inter=list(v['interludes'])
        for i,g in enumerate(v['games']):
            ps=[('In one sentence',g.get('quick_pitch',g['pleasure'])),('What you do',g.get('what_you_do',g['first'])),('How you play',control_line(g)),('Main catch',g.get('watch_for',g['burden'])),('Edition considered',g.get('edition_target','See the evidence note for the edition.')),('Editorial starting point',metadata_line(g)),('Why it might click',g['pleasure']),('The full catch',g['burden']),('Try this first',g['first']),('Aiden after hours',g['case_title'])]+[(None,p)for p in g['scene']]+[('Aiden’s verdict',g['ruling']),('Evidence desk',g['source_note'])]+[('Source',u)for u in g['source_urls']]
...
def main():
    ...
    corpus={'title':TITLE,'edition':'2.0 - The Tellings Cut','checked':DATE,'scope':'240 editorial candidates, including conditional and detour entries; no full-game accessibility certification','privacy':'No personal or clinical history included','games':games}
